...
function print_suchliste($ID, $row ,$bib_id)
//==========================================================================
// Function     : print_liste
//--------------------------------------------------------------------------
// Beschreibun  : Darstelen einer Datenliste  mit
...
//==========================================================================
{
	...
	echo $row[1]."/".$row[2]."/".$row[3]."/".$row[4]."/".$row[5]."/".$recht."/".$zugriff;
	...
	if ($row[3] > "" OR $row[4] > "" OR $row[5] > "")
	{
		...
		if ($row[3] > "" )
		{
			$where_like = $where_like."item like \"%$row[3]%\"  ";
		}
		if ($row[4] > "" )
		{
			if ($row[3] > "" )
			{
				$where_like = $where_like." OR  ";
			}
			$where_like = $where_like."titel like \"%$row[4]%\" ";
		}
		if ( $row[5] > "")
		{
			if ($row[3] > "" OR $row[4] > "" )
			...
			$where_like = $where_like."kurz like \"%$row[5]%\" ";
		}
		$where_like = $where_like.") ";
	}


	$where = "where bereich=\"$row[1]\" AND thema like \"%$row[2]%\" $where_like $where_and ";
...
function print_suchen($next,$ID,$bib_id)
{
	//==========================================================================
	// Function     :  print_suchen
	//--------------------------------------------------------------------------
	// Beschreibung : Bearbeiten der Felder der Tabelle
	//                Die Abfrage MUSS alle Felder beinhalten
	//                linke Spalte = Feldnamem
	...
	echo "<INPUT TYPE=\"hidden\" NAME=\"bib_id\" VALUE=\"$bib_id\">\n";
	echo "<INPUT TYPE=\"hidden\" NAME=\"ID\"  VALUE=\"$ID\">\n";
	echo "<TABLE WIDTH=\"600\" BORDER=\"1\"  CELLPADDING=\"1\" CELLSPACING=\"2\" BGCOLOR=\"\" BORDERCOLOR=\"#EDDBCB\"
			BORDERCOLORDARK=\"silver\" BORDERCOLORLIGHT=\"#ECD8C6\">\n";
	echo "\t<tr>\n";
	echo "\t<td width=50></td>\n";
	echo "\t<td width=500><center><b>$TABLE</b></td>\n";
	echo "\t</tr>\n";
	for ($i=1; $i<6; $i++)
	{
		switch($i):
		case 1:
			echo "<tr>";
		echo "\t<td >Bereich&nbsp;</td>\n";
		echo "<td width=500><input type=\"text\" name=\"row[1]\" SIZE=10 VALUE=\"$row[$i]\" readonly></td>\n";
		echo "<tr>";
		break;
		case 2 :
			echo "<tr>";
		...
		case 3 :
			...
			echo "\t<td >Item&nbsp;</td>\n";
			echo "\t<td>\n";
			echo "<input type=\"text\" name=\"row[3]\" SIZE=25  VALUE=\"\">\n";
			echo "\t &nbsp;&nbsp;Liste der vordefinierten <select name=\"1\" MAXLENGTH=25 >\n";
			echo "\t<option selected> $row[$i]\n";
			for ($j=0; $j<$item_anz; $j++)
			{
				$item = mysql_fetch_array ($result2);
		...
		case 4:
			...
			echo "<td><input type=\"text\" name=\"row[4]\" SIZE=50  VALUE=\"\"></td>\n";
		...
		case 5:
			...
			echo "<td><input type=\"text\" name=\"row[5]\" SIZE=50  VALUE=\"\"></td>\n";
...
function make_such_menu($THEMEN,$ID,$bib_id)
//==========================================================================
...
//  2=>array("icon" => "1","caption" => "Tag 1","link" => "$PHP_SELF?md=11&ID=$ID"),
{
	...
	$menu[2] = array("icon" => "0","caption" => "","link" => "$PHP_SELF?md=0&ID=$ID");
	$menu[3] = array("icon" => "99","caption" => "SUCHEN","link" => "$PHP_SELF?md=10&ID=$ID&THEMEN=%&bib_id=$bib_id");

	$menu[50] = array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=9&ID=$ID&bib_id=$bib_id");
...
//  2=>array("icon" => "1","caption" => "Tag 1","link" => "$PHP_SELF?md=11&ID=$ID"),
{
	...
	if ($recht == "write")
	{
		$menu[1] = array("icon" => "2","caption" => "Erfassen","link" => "$PHP_SELF?md=1&ID=$ID&THEMEN=$THEMEN&bib_id=$bib_id");
	}
	$menu[2] = array("icon" => "0","caption" => "","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=%&bib_id=$bib_id");
	$menu[3] = array("icon" => "1","caption" => "SUCHEN","link" => "$PHP_SELF?md=10&ID=$ID&THEMEN=%&bib_id=$bib_id");
...
*/
{
	...
	$menu[50] = array("icon" => "6","caption" => "Zurück","link" => "larp.php?md=0&ID=$ID&bib_id=$row[2]");
...
// 		Code ausgeführt wird. */
		break;
	case 11:  // Das Basis Menü mit den THEMEN
		$md=11;
		break;
	default :
		break;
...
// 		Code ausgeführt wird. */
	...
	case 9:  // Das Basis Menü mit den THEMEN
		...
		break;
	case 10:  // Das Basis Menü mit den THEMEN
		$menu = make_such_menu($THEMEN,$ID,$bib_id);
		print_menu($menu);
		print_suchen(11,$ID,$bib_id);
		break;
	case 11:  // Das Basis Menü mit den THEMEN
		$menu = make_such_menu($THEMEN,$ID,$bib_id);
		print_menu($menu);
		print_suchliste($ID, $p_row ,$bib_id);
		break;
	default:  // Das Basis Menü mit den THEMEN
		$menu = make_bereich_menu($THEMA,$ID);
		print_menu($menu);
		print_bereiche($ID, $THEMA,$bib_id);
		break;
	endswitch;
