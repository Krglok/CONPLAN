...
/**
 * Erstellt einen leeren Datensatz und ruft die Detailmaske auf
 * @param unknown $id		, PK des spielers
 * @param unknown $ID		, session ID
 * @param unknown $next , zurufende Datenbank Funktion
 */
...
/**
 * Erstellt eine Spielerliste zur selektion eines Spielers
 * dieser wird in der Erfassungsmaske als Referenz verwendet
 * @param unknown $ID, session ID
 */
  ...
  while ($row = mysql_fetch_row($result))
  {
    ...
    for ($i=0; $i<$field_num; $i++)
    {
      ...
      if ($i==0)
      {
        ...
        print_menu_icon ("_tcheck","Auswahl des Spielers");
        echo "\t</a></td>\n";
      } else
      {
        echo "\t<td>$row[$i]&nbsp;</td>\n";
