...
function print_fert($ID,$char)
{
	...
	$where = ' where kapitel="2" AND absatz="3" AND item<>"0" ';
...
function get_menu_char_waf($md, $PHP_SELF, $ID, $titel, $id, $daten, $sub, $home)
{
  switch ($md):
  case char_add: // Erfassen eines neuen Datensatzes
    $menu = array (0=>array("icon" => "99","caption" => "WAFFEN","link" => ""),
...
// ----------------------------------------------------------------
  ...
  $titel = "Charakterdaten";
	...
	$menu = get_menu_char_waf($md, $PHP_SELF, $ID, $titel, $id, $daten, $sub, $home);
