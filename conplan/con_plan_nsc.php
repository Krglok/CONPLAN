...
function print_liste($ID)
{
	...
	$q = "select id,S0,s1,name,kurz,r_grp,notority,hintergrund from $TABLE where S0=\"$TAG\" order by S0,S1";
...
function print_loeschen($ID)
{
	...
	//$q = "select id,S0,s1,name,kurz,r_grp,r_char,notority from $TABLE where S0=$TAG";
	$q = "select id,S0,s1,name,kurz,r_grp,notority from $TABLE where S0=\"$TAG\" order by S0,S1";
	$result = mysql_query($q)  or die("Query Fehler...$q");
...
function print_info($id,$ID)
{
	...
	//Anzeigen von NSC Daten als einfache Maske
	//function view() {
...
echo "<!--  DATEN Spalte   -->\n";
	...
	for ($i=0; $i<$field_num; $i++)
	{
		...
		} else
		{
			echo "<tr>";
			echo "\t<td><b></b></td>\n";
			echo "\t<td><TEXTAREA NAME=\"$field_name[$i]\" COLS=80 ROWS=55 readonly>$row[$i]</TEXTAREA>&nbsp;</td>\n";
			echo "<tr>";
		}
	}
	echo "</table>";
	...
	for ($i=0; $i<$field_num; $i++)
	{
		...
		if ($i!=0)
		{
			...
			} else
			{
				echo "<tr>";
				echo "\t<td><b></b></td>\n";
				echo "\t<td><TEXTAREA NAME=\"row[$i]\" COLS=80 ROWS=55>$row[$i]</TEXTAREA>&nbsp;</td>\n";
				echo "<tr>";
			}
		} else
		{
...
case 1: // Erfassen eines neuen Datensatzes
		$menu = array (0=>array("icon" => "1","caption" => "NSC $TAG","link" => ""),
...
default:  // MAIN-Menu
	...
	3=>array("icon" => "_printer","caption" => "Report","link" => "nsc_rep.php?md=0&ID=$ID&TAG=$TAG"),
