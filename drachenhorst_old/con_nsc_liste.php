...
//==========================================================================
{
	...
	$where = "";
	$order = "where nsc=\"J\"";
	$q = "select id,name,beruf,rasse,gilde,nsc,aktiv,wappen_path,bild_path from $TABLE $where $order ";
	...
	while ($row = mysql_fetch_row($result))
	{
		...
		//echo "\t<IMG SRC=\"../larp/images/xview.gif\" BORDER=\"0\" HEIGHT=\"15\" WIDTH=\"25\" ALT=\"Fertigkeiten  bearbeiten\" HSPACE=\"0\" VSPACE=\"0\">\n";
		print_menu_icon (5);
		echo "\t</a></td>\n";
		echo "</tr>";
	}
...
function print_maske($id,$ID,$next,$erf)
{
	...
	for ($i=0; $i<$field_num; $i++)
	{
		...
		if (($i!=0) AND ($i!=1) AND ($i!=13) AND ($i!=12)  AND ($i!=7) AND ($i!=8) AND ($i!=9))
		{
			if ($type[$i]!="blob")
			{
				echo "<tr>";
...
default :
	...
	switch ($md):
case 0:  // MAIN-Menu
		$menu = array (0=>array("icon" => "99","caption" => "NSC-LISTE","link" => ""),
				2=>array("icon" => "2","caption" => "Neuer NSC","link" => "$PHP_SELF?md=1&ID=$ID"),
...
case 2: // ANSEHEN Form
	...
	2=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=0&ID=$ID&TAG=$TAG&LISTE=$LISTE")
	);
	print_menu($menu);
