...
//-----------------------------------------------------------------------------
function print_liste($ID, $THEMEN)
//==========================================================================
...
//==========================================================================
{
	...
	$where = "where bereich=\"GDW\" AND thema like \"$THEMEN\" AND zugriff =\"public\"";
	...
	while ($row = mysql_fetch_row($result))
	{
		...
		for ($i=0; $i<$field_num; $i++)
		{
			...
			{
				echo "\t<td>";
				//        echo "<a href=\"$PHP_SELF?md=4&ID=$ID&id=$row[$i]&THEMEN=$THEMEN\">\n";
				//        print_menu_icon (3);
				echo " ";
				//        echo "\t</a>
				echo "</td>\n";
		...
		echo "\t<td><a href=\"$PHP_SELF?md=2&ID=$ID&id=$row[0]&THEMEN=$THEMEN\">\n";
		print_menu_icon (7);
		echo "\t</a></td>\n";
		echo "<tr>";
	}
...
function print_loeschen($THEMEN)
//==========================================================================
...
//==========================================================================
{
	...
	$q = "select * from $TABLE where thema=\"$THEMEN\"";
	$result = mysql_query($q)  or die("Query Fehler...");
...
function print_info($id,$THEMEN,$ID)
//==========================================================================
// Function     : print_info
//--------------------------------------------------------------------------
...
//==========================================================================
{
	...
	echo "\t</td>\n";
	echo "\t<td > \n";
	echo "\t<a href=\"$PHP_SELF?md=0&ID=$ID&THEMEN=$THEMEN\"> \n";
...
function print_maske($id,$THEMEN,$next,$erf,$ID)
{
	//==========================================================================
...
function make_thema_menu($THEMEN,$ID)
//==========================================================================
...
//  2=>array("icon" => "1","caption" => "Tag 1","link" => "$PHP_SELF?md=11&ID=$ID"),
{
	...
	$menu = array (0=>array("icon" => "99","caption" => "GDW-BIBLIOTHEK","link" => ""),
			1=>array("icon" => "2","caption" => "Einreichen","link" => "$PHP_SELF?md=1&ID=$ID&THEMEN=$THEMEN"),
			2=>array("icon" => "7","caption" => "Benutzuung","link" => "$PHP_SELF?md=5&ID=$ID&THEMEN=$THEMEN&daten=gdw_benutzung.html"),
			3=>array("icon" => "0","caption" => "THEMEN","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=%"),
			4=>array("icon" => "1","caption" => "ALLE","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=%")
	);
	for ($i=1; $i<$anz; $i++)
	{
		$row = mysql_fetch_row($result);
		//    $menu = array($i=>array("icon" => "1","caption" => "$row[1]","link" => "$PHP_SELF?md=11&TAG=$row[2]"));
		$menu[$i+5] = array("icon" => "1","caption" => "$row[1]","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=$row[1]");
	}
	//  $menu[49] = array("icon" => "4","caption" => "Erfassen","link" => "$PHP_SELF?md=1&ID=$ID");
	$menu[50] = array("icon" => "6","caption" => "Zurück","link" => "larp.php?md=0&ID=$ID");
...
case 5:  // MAIN-Menu
	insert($p_row);
header ("Location: $PHP_SELF?md=11&ID=$ID&THEMEN=$THEMEN");  /* Umleitung des Browsers
zur PHP-Web-Seite. */
exit;  /* Sicher stellen, das nicht trotz Umleitung nachfolgender
Code ausgeführt wird. */
break;
...
case 7: // Delete eines bestehenden Datensatzes
		// SQL delete
		loeschen($id);
	header ("Location: $PHP_SELF?md=3&ID=$ID&THEMEN=$THEMEN");  /* Umleitung des Browsers
	zur PHP-Web-Seite. */
	exit;  /* Sicher stellen, das nicht trotz Umleitung nachfolgender
	Code ausgeführt wird. */
	break;
default :
	...
	print_kopf(3,4,"SL Bereich","Sei gegrüsst Meister ");
...
case 1: // Erfassen eines neuen Datensatzes
		$menu = array (0=>array("icon" => "99","caption" => "GDW-BIBLIOTHEK","link" => ""),
				1=>array("icon" => "99","caption" => "EINREICHEN","link" => ""),
				2=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=$THEMEN")
		);
		print_menu($menu);
		print_maske($id,$THEMEN,5,1,$ID);
		break;
case 2: // ANSEHEN Form
	$menu = array (0=>array("icon" => "99","caption" => "GDW-BIBLIOTHEK","link" => ""),
	1=>array("icon" => "99","caption" => "ANSEHEN","link" => ""),
	2=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=11&ID=$ID&THEMEN=$THEMEN")
	);
	//      print_menu($menu);
	Print_info($id, $THEMEN,$ID);
	break;
case 3: // Delete eines bestehenden Datensatzes
	$menu = array (0=>array("icon" => "99","caption" => "GDW-BIBLIOTHEK","link" => ""),
	1=>array("icon" => "99","caption" => "LÖSCHEN","link" => ""),
	9=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=$THEMEN")
	);
	print_menu($menu);
	break;
case 4: // Bearbeiten Form
	...
	print_menu($menu);
	print_maske($id, $THEMEN,6,0,$ID);
	break;
case 5: // MAIN MEN?
	$menu = make_thema_menu($THEMEN,$ID);
	print_menu($menu);
	print_data($daten);
	break;
