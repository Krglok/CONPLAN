...
beschreibung : realisiert die Bearbeitungsfunktionen für die Orte der Planung
...
function print_liste($ID)
{
	...
	$q = "select ID,S0,S1,S2,name,bezeichnung,magie,r_nsc from $TABLE where S0=\"$TAG\" order by S0,S1,S2";
...
function print_info($id,$ID)
{
	...
	for ($i=0; $i<$field_num; $i++)
	{
		...
		} else
		{
			echo "<tr>";
			echo "\t<td><b></b></td>\n";
			echo "\t<td><TEXTAREA NAME=\"$field_name[$i]\" COLS=50 ROWS=12 readonly>$row[$i]</TEXTAREA>&nbsp;XX</td>\n";
			echo "<tr>";
		}
	}
	echo "</table>";
...
default :
	...
	print_kopf(9,0,"Con Planung Orte","Sei gegrüsst Meister ");
...
case 1: // Erfassen eines neuen Datensatzes
		$menu = array (0=>array("icon" => "99","caption" => "ORTE Tag $TAG","link" => ""),
...
default:  // MAIN-Menu
	...
	3=>array("icon" => "5","caption" => "Drucken","link" => "orte_rep?md=0&ID=$ID&TAG=$TAG"),
