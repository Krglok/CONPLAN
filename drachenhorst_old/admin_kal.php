...
beschreibung : realisiert die Bearbeitungsfunktionen für die Datei <kalender>
...
//==========================================================================
{
	...
	$q = "select * from $TABLE order by KAL_ID DESC LIMIT 52";
...
//==========================================================================
{
	...
	$q = "select * from $TABLE where KAL_ID=$id";
...
//==========================================================================
{
	...
	/**/
	$q = "delete from $TABLE where KAL_ID=\"$id\" ";
	//  echo $q;
...
function print_maske($id,$ID,$next,$erf)
{
	...
	global $Gjahr ;	// globale Zwischenspeicher fuer Kalendererafssung
	global $Gmonat;	// globale Zwischenspeicher fuer Kalendererafssung
	global $Gkw   ;	// globale Zwischenspeicher fuer Kalendererafssung
	global $GTag  ;	// globale Zwischenspeicher fuer Kalendererafssung
	...
	} else
	{
		...
		$q = "select * from $TABLE order by KAL_ID DESC LIMIT 1";
		$result = mysql_query($q)  or die("Query Fehler...");
		$row = mysql_fetch_array ($result);

		$Gjahr = $row[1];	// globale Zwischenspeicher fuer Kalendererafssung
		$Gmonat= $row[3];	// globale Zwischenspeicher fuer Kalendererafssung
		$Gkw   = $row[2];	// globale Zwischenspeicher fuer Kalendererafssung
		$GTag  = $row[10];	// globale Zwischenspeicher fuer Kalendererafssung

		$q = "select * from $TABLE where KAL_ID=\"0\"";
	...
	if (count($row)==1)
	{
		...
		$row[1]=$Gjahr;	// globale Zwischenspeicher fuer Kalendererafssung
		$row[3]=$Gmonat;	// globale Zwischenspeicher fuer Kalendererafssung
		$row[2]=$Gkw+1  ;	// globale Zwischenspeicher fuer Kalendererafssung
		$row[4]=$GTag+1 ;	// globale Zwischenspeicher fuer Kalendererafssung
		$row[5]=$GTag+2 ;	//  Berechnung der folge tage
		$row[6]=$GTag+3 ;	//  Berechnung der folge tage
		$row[7]=$GTag+4 ;	//  Berechnung der folge tage
		$row[8]=$GTag+5 ;	//  Berechnung der folge tage
		$row[9]=$GTag+6 ;	//  Berechnung der folge tage
		$row[10]=$GTag+7 ;	//  Berechnung der folge tage
	};
	/**/
		...
		if ($i!=0)
		{
			if ($type[$i]!="blob")
			{
				if (($i>=11) & ($i<=17))
				{
					...
					echo " [nocon,contag,aufbau,basteln,vv] </td>\n";
...
default:  // die einzelnen Bildseiten 11-xx
	...
	3=>array ("icon" => "1","caption" => "Löschen","link" => "$PHP_SELF?md=3&ID=$ID"),
