...
//-----------------------------------------------------------------------------
function print_liste($ID, $THEMEN,$BIB)
//==========================================================================
...
//==========================================================================
{
	...
	$where = "where bereich=\"$BIB\" AND thema like \"$THEMEN\" ";
	...
	while ($row = mysql_fetch_row($result))
	{
		...
		for ($i=0; $i<$field_num; $i++)
		{
			...
			if ($i==0)
			{
				echo "\t<td><a href=\"$PHP_SELF?md=4&ID=$ID&id=$row[$i]&THEMEN=$THEMEN&BIB=$BIB\">\n";
				print_menu_icon (3);
				echo "\t</a></td>\n";
			} else
			{
		...
		echo "\t<td><a href=\"$PHP_SELF?md=2&ID=$ID&id=$row[0]&THEMEN=$THEMEN&BIB=$BIB\">\n";
		print_menu_icon (7);
		echo "\t</a></td>\n";
		echo "<tr>";
	}
...
function print_info($id,$THEMEN,$ID,$BIB)
//==========================================================================
// Function     : print_info
//--------------------------------------------------------------------------
...
//==========================================================================
{
	...
	echo "\t</td>\n";
	echo "\t<td > \n";
	echo "\t<a href=\"$PHP_SELF?md=9&ID=$ID&THEMEN=$THEMEN&BIB=$BIB\" > \n";
...
function print_maske($id,$THEMEN,$next,$erf,$ID,$BIB)
{
	//==========================================================================
	...
	if (count($row)==1)
	{
		...
		$row[1] = $BIB;		// Vorbelegter Wert Bibliothek
		$row[2] = $THEMEN;	// Vorbelegter Wert Thema
	};
	/**/
	...
	echo "<INPUT TYPE=\"hidden\" NAME=\"THEMEN\" VALUE=\"$THEMEN\">\n";
	echo "<INPUT TYPE=\"hidden\" NAME=\"BIB\" VALUE=\"$BIB\">\n";
...
function make_thema_menu($THEMEN,$ID,$BIB)
//==========================================================================
...
//  2=>array("icon" => "1","caption" => "Tag 1","link" => "$PHP_SELF?md=11&ID=$ID"),
{
	...
	$menu = array (0=>array("icon" => "99","caption" => "$BIB","link" => ""),
			1=>array("icon" => "11","caption" => "Erfassen","link" => "$PHP_SELF?md=1&ID=$ID&THEMEN=$THEMEN&BIB=$BIB"),
			2=>array("icon" => "0","caption" => "","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=%&BIB=$BIB"),
			3=>array("icon" => "1","caption" => "ALLE","link" => "$PHP_SELF?md=9&ID=$ID&THEMEN=%&BIB=$BIB")
	);
	for ($i=4; $i<$anz+3; $i++)
	{
		$row = mysql_fetch_row($result);
		//    $menu = array($i=>array("icon" => "1","caption" => "$row[1]","link" => "$PHP_SELF?md=11&TAG=$row[2]"));
		$menu[$i] = array("icon" => "1","caption" => "$row[1]","link" => "$PHP_SELF?md=9&ID=$ID&THEMEN=$row[1]&BIB=$BIB");
	}
	//  $menu[49] = array("icon" => "4","caption" => "Erfassen","link" => "$PHP_SELF?md=1&ID=$ID");
	$menu[50] = array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=0&ID=$ID&BIB=$BIB");

	return  $menu;

};

function make_bib_menu($THEMEN,$ID)
//==========================================================================
// Function     :  make_bild_menu
//--------------------------------------------------------------------------
// Beschreibung : Erstellt das Bildgalerie Menü aus der Tabelle <bilder_topic>
...
//  2=>array("icon" => "1","caption" => "Tag 1","link" => "$PHP_SELF?md=11&ID=$ID"),
{
	...
	$q = "select * from $TABLE3 order by bereich";
	$result = mysql_query($q)  or die("Query Menu...");
	$anz = mysql_num_rows($result);
	mysql_close($db);


	$menu = array (0=>array("icon" => "99","caption" => "ADMIN-BIBLIOTHEK","link" => ""),
	...
	{
		$row = mysql_fetch_row($result);
		$menu[$i+4] = array("icon" => "1","caption" => "$row[1]","link" => "$PHP_SELF?md=9&ID=$ID&BIB=$row[1]");
	}
	$menu[50] = array("icon" => "6","caption" => "Zurück","link" => "admin_con.php?md=0&ID=$ID&BIB=$BIB");
...
case 5:  // MAIN-Menu
	insert($p_row);
header ("Location: $PHP_SELF?md=11&ID=$ID&THEMEN=$THEMEN&BIB=$BIB");  /* Umleitung des Browsers
zur PHP-Web-Seite. */
exit;  /* Sicher stellen, das nicht trotz Umleitung nachfolgender
Code ausgeführt wird. */
break;
...
case 7: // Delete eines bestehenden Datensatzes
		// SQL delete
		loeschen($id);
	header ("Location: $PHP_SELF?md=3&ID=$ID&THEMEN=$THEMEN&BIB=$BIB");  /* Umleitung des Browsers
	zur PHP-Web-Seite. */
	exit;  /* Sicher stellen, das nicht trotz Umleitung nachfolgender
	Code ausgeführt wird. */
	break;
...
case 1: // Erfassen eines neuen Datensatzes
		$menu = array (0=>array("icon" => "99","caption" => "ADMIN-BIBLIOTHEK","link" => ""),
				...
				2=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=$THEMEN&BIB=$BIB")
		);
		print_menu($menu);
		print_maske($id,$THEMEN,5,1,$ID,$BIB);
		break;
case 2: // ANSEHEN Form
	$menu = array (0=>array("icon" => "99","caption" => "ADMIN-BIBLIOTHEK","link" => ""),
	1=>array("icon" => "99","caption" => "ANSEHEN","link" => ""),
	2=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=11&ID=$ID&THEMEN=$THEMEN&BIB=$BIB")
	);
	//      print_menu($menu);
	Print_info($id, $THEMEN,$ID,$BIB);
	break;
case 3: // Delete eines bestehenden Datensatzes
	$menu = array (0=>array("icon" => "99","caption" => "ADMIN-BIBLIOTHEK","link" => ""),
	1=>array("icon" => "99","caption" => "LÖSCHEN","link" => ""),
	9=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=0&ID=$ID&THEMEN=$THEMEN&BIB=$BIB")
	);
	print_menu($menu);
	break;
case 4: // Bearbeiten Form
	...
	2=>array("icon" => "6","caption" => "Zurück","link" => "$PHP_SELF?md=9&ID=$ID&THEMEN=$THEMEN&BIB=$BIB")
	);
	print_menu($menu);
	print_maske($id, $THEMEN,6,0,$ID,$BIB);
	break;
case 9:  // Das Basis Menü mit den THEMEN
	$menu = make_thema_menu($THEMEN,$ID,$BIB);
	print_menu($menu);
	print_liste($ID, $THEMEN,$BIB);
	break;
default:  // Das Basis Menü mit den THEMEN
	$menu = make_bib_menu($THEMEN,$ID);
	print_menu($menu);
	print_bereiche($ID, $THEMEN,$BIB);
	break;
	endswitch;
