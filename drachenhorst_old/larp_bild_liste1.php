...
//  2=>array("icon" => "1","caption" => "Tag 1","link" => "$PHP_SELF?md=11&ID=$ID"),
{
	...
	$q = "select * from $TABLE1 where tag>99 order by tag DESC,sort DESC";
	...
	{
		$row = mysql_fetch_row($result);
		$menu[$i+1] = array("icon" => "1","caption" => "$row[1]","link" => "$PHP_SELF?md=11&ID=$ID&TAG=$row[2]&LISTE=$row[1]");
	}
