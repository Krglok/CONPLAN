...
beschreibung : realisiert die Bearbeitungsfunktionen für die Datei <bilder_topic>
...
row[3] = path > wird fest vergeben.
Das automatische anlegen von Pfaden ist problematisch
und wird deshalb nicht benutzt.
Die Bilder werden in einen Standardpfad gepackt.
...
$Log: bilder_admin.php,v $
Revision 1.2  2002/06/01 12:39:29  windu
korrektur ; kein mkdir !!
...
//==========================================================================
{
	...
	$q = "select * from $TABLE order by tag DESC,sort";
...
//==========================================================================
{
	...
	$q = "select * from $TABLE order by tag,sort";
	$result = mysql_query($q)  or die("Query Fehler...");
	...
	while ($row = mysql_fetch_row($result))
	{
		...
		echo "\t<td><a href=\"$PHP_SELF?md=3&ID=$ID&id=$row[0] \">\n";
		print_menu_icon (7);
		echo "\t</a></td>\n";
		echo "<tr>";
	}
...
function print_maske($id,$ID,$next,$erf)
{
	...
	echo "<B> ACHTUNG neue Pfade müssen per FTP angelegt werden !!!<BR>\n";
	echo " Leeres Feld \"path\" = Standardpfad!!!\n";
	echo "</B> \n";
	echo "<FORM ACTION=\"$PHP_SELF\" METHOD=POST>\n";
...
$TABLE  = "bilder_topic";    // Haupttabelle für das Modul
...
default:  // MAIN-Menu
	$menu = array (0=>array("icon" => "99","caption" => "Bilder Topic","link" => ""),
